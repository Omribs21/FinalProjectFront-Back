...
def register(request): # request =  {"username":"omri","password":1234}
    ...
    all_users = User.objects.all() # retunes all the users in db
    for x in range(len(all_users)): # returens if the user name is taken
       if str(all_users[x]) == Username:
            return Response({"taken":"true"})
    print(Username, Password, Email)
    user = User.objects.create_user(
        username=Username, password=Password, email=Email,first_name= First_name, last_name= Last_name)
    return JsonResponse({"first name": First_name, "last name": Last_name})
